...
def _network_preflight_with_retry(email: str, proxy: str | None, max_attempts: int = 6) -> tuple[BrowserSession, str]:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        fingerprint_text = getattr(session, "fingerprint_summary_text", None)
...
def check_account_liveness(email: str, proxy: str | None = None, *, clear_log: bool = True) -> dict:
    ...
    try:
        ...
        fingerprint_summary = getattr(session, "fingerprint_summary", None)
        fingerprint_text = getattr(session, "fingerprint_summary_text", None)
        fp = fingerprint_summary() if callable(fingerprint_summary) else {}
        return {
            "ok": True,
            "status": "live",
